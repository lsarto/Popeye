...
public class SearchController {
	...
	public String searchByCategory(@RequestParam("category") String category, Model model, Principal principal) {
		...
		if (category.equals("all")) {
			cigaretteList = cigaretteService.findAll();
		} else {
			cigaretteList = cigaretteService.findByCategory(category);
		}
	...
	public String searchCigarette(@ModelAttribute("keyword") String keyword, Principal principal, Model model) {
		if (principal != null) {
			String username = principal.getName();
			User user = userService.findByUsername(username);
			model.addAttribute("user", user);
