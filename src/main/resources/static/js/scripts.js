	...
	if(password == "" && confirmPassword =="") {
		...
		if(password != confirmPassword) {
			...
			if(password.length<8){
				$("#checkPasswordMatch").html("La password deve avere almeno 8 caratteri");
				$("#updateUserInfoButton").prop('disabled', true);
			} else if(!(new RegExp(passLowerCase)).test(password)) {
				$("#checkPasswordMatch").html("La password deve avere almeno una lettera minuscola");
				$("#updateUserInfoButton").prop('disabled', true);
			} else if(!(new RegExp(passUpperCase)).test(password)) {
				$("#checkPasswordMatch").html("La password deve avere almeno una lettera maiuscola");
				$("#updateUserInfoButton").prop('disabled', true);
			} else if(!(new RegExp(passNum)).test(password)) {
				$("#checkPasswordMatch").html("La password deve avere almeno un numero");
				$("#updateUserInfoButton").prop('disabled', true);
			}
		} else {
			$("#checkPasswordMatch").fadeOut();
...
function checkEmailFormat(){
	...
	if(!pattEmail.test(emailLogin)) {
		...
		$("#checkEmailFormat").html("Il formato dell'indirizzo email non è corretto");
...
$(document).ready(function(){
	...
	$("#email-login").keyup(checkEmailFormat); //validazione email in customer-register.html
});